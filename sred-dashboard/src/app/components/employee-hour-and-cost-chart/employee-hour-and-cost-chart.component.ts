...
/**
 * Employee SR&ED hours & cost chart: per employee, SR&ED hours (columns, left axis)
 * and SR&ED labor cost (line, right axis), where cost = SR&ED hours × the employee's
 * hourly rate. Only SR&ED-eligible hours are included. Period-reactive; stable-reference
 * chart inputs (vm).
 */
@Component({
  selector: 'app-employee-hour-and-cost-chart',
  standalone: true,
  imports: [AsyncPipe, NgApexchartsModule],
  templateUrl: './employee-hour-and-cost-chart.component.html',
})
export class EmployeeHourAndCostChartComponent {
  private readonly data = inject(DashboardDataService);

  readonly vm$ = this.data.employeeCostBreakdown$.pipe(
    map((rows) => ({
      empty: rows.length === 0,
      series: [
        { name: 'SR&ED hours', type: 'column', data: rows.map((r) => r.hours) },
        { name: 'SR&ED cost', type: 'line', data: rows.map((r) => Math.round(r.amount)) },
