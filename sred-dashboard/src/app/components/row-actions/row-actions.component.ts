...
/**
 * Edit (pencil) + delete (trash) icon buttons for a grid row (P3.4b). Icon-only for a
 * cleaner, more prominent look than text links; each emits an event the grid handles.
 */
@Component({
  ...
  template: `
    <div class="inline-flex items-center gap-1">
      ...
      <button
        ...
        title="Delete"
        aria-label="Delete"
        class="rounded-lg p-1.5 text-gray-400 transition-colors hover:bg-danger/10 hover:text-danger"
      >
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.6" stroke="currentColor" class="h-4 w-4">
